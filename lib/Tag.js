...
class Tag extends Component {
  ...
  render = () => {
    ...
    const { connectDragSource, isDragging, connectDropTarget, readOnly, handleTagClick } = props;
    ...
    const tagComponent = (
      <span style={{opacity: isDragging ? 0 : 1}} className={props.classNames.tag}>
        <span className={props.classNames.tagLabel} onClick={handleTagClick.bind(null, props.tag)}>{label}</span>
