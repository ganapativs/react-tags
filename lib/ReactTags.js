...
class ReactTags extends React.Component {
    ...
    filteredSuggestions = (query, suggestions) => {
        ...
        return suggestions.filter(function (item) {
            return (Object.prototype.toString.call(item) === '[object Object]' ? item.text : item)
    ...
    handlePaste = (e) => {
        ...
        string.split(regExp).forEach((tag) => this.props.handleInputChange ? this.handleChange({target: {value: string}}): this.props.handleAddition(tag))
    };

    addTag = (tag) => {
        const {handleBeforeAddition, autocomplete, suggestions, handleAddition} = this.props;
        let {input} = this.refs;

        if (autocomplete) {
            const possibleMatches = this.filteredSuggestions(tag, suggestions);

            if ((autocomplete === 1 && possibleMatches.length === 1) ||
